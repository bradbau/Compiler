...
TAC:: MergeTACItem(TAC* A, TAC* B)     //将类对象作为函数的形参，对象里包括三元式和间接码表(我不会用arg_list就只弄两个TAC的合并吧)
{
    int i = 1;
    for (i = 0; i < B->code_table.size(); i++) {
        A->code_table.push_back(B->code_table[i] + A->code_table.size());
        B->TAC_table[i].seq += A->code_table.size();
        A->TAC_table.push_back(B->TAC_table[i]);
    }
    return A;
}
...
{
    TAC code1(NULL, NULL, NULL);
    TAC code2(NULL, NULL, NULL);
    TAC code3(NULL, NULL, NULL);
    TAC code4(NULL, NULL, NULL);
    TAC code5(NULL, NULL, NULL);
    TAC code6(NULL, NULL, NULL);
    TAC code7(NULL, NULL, NULL);
    if (!strcmp(tree->lchild->name, "LVal")) {

    }
    else if (!strcmp(tree->lchild->name, "SPSEMICOLON")) {

    
    else if (!strcmp(tree->lchild->name, "Exp")) {

    }
    else if (!strcmp(tree->lchild->name, "Block")) {

    }
    else if (!strcmp(tree->lchild->name, "KEYIF")) {
        Label label1 = label_num++;
        Label label2 = label_num++;
        GrammarTree cond = tree->lchild->rchild->rchild;
        GrammarTree stmt = tree->lchild->lchild->rchild->rchild->rchild;
        scope.localscope = GetStackTop(stack)->localscope;
        code1 = TranslateCondition(cond, scope, stack, label1, label2);
        scope.localscope = GetStackTop(stack)->localscope;
        code2 = TranslateStmt(stmt, scope, stack);
        code3.code_table.push_back(0);
        code3.TAC_table[0].seq = 0;
        code3.TAC_table[0].optype = LABELDF;
        code3.TAC_table[0].firstOp.Type = LABEL;
        code3.TAC_table[0].firstOp.value = label1;
        code4.code_table.push_back(0);
        code4.TAC_table[0].seq = 0;
        code4.TAC_table[0].optype = LABELDF;
        code4.TAC_table[0].firstOp.Type = LABEL;
        code4.TAC_table[0].firstOp.value = label2;
        if (tree->lchild->rchild->rchild->rchild->rchild->rchild!=NULL && !strcmp(tree->lchild->rchild->rchild->rchild->rchild->rchild->name, "KEYELSE"))
        {
            Label label3 = label_num++;
            scope.localscope = GetStackTop(stack)->localscope;
            code5 = TranslateStmt(stmt->rchild->rchild, scope, stack);
            code6.code_table.push_back(0);
            code6.TAC_table[0].seq = 0;
            code6.TAC_table[0].optype = GOTO;
            code6.TAC_table[0].firstOp.Type = LABEL;
            code6.TAC_table[0].firstOp.value = label3;
            code7.code_table.push_back(0);
            code7.TAC_table[0].seq = 0;
            code7.TAC_table[0].optype = LABELDF;
            code7.TAC_table[0].firstOp.Type = LABEL;
            code7.TAC_table[0].firstOp.value = label3;
            return MergeTACItem(MergeTACItem(MergeTACItem(code1, code3), MergeTACItem(code2, code6)), MergeTACItem(MergeTACItem(code4, code5), code7));
            //return MergeTACCode(7, code1, code3, code2, code6, code4, code5, code7);
        }
        else
            return MergeTACCode(MergeTACItem(code1, code3), MergeTACItem(code2, code4));
    }
    else if (!strcmp(tree->lchild->name, "KEYWHILE")) {

    }
    else if (!strcmp(tree->lchild->name, "KEYBREAK")) {

    }
    else if (!strcmp(tree->lchild->name, "KEYCONTINUE")) {

    }
    else if (!strcmp(tree->lchild->name, "KEYRETURN")) {

    }
    else return TAC(tree->lchild, scope, stack); 


    else if (!strcmp(tree->lchild->name, "IfStmt"))
    {
...
TAC::TAC(GrammarTree tree, Scope scope, ScopeStack* stack) {
    TAC fun(NULL, NULL, NULL);          //函数定义部分用到的TAC对象
    TAC var(NULL, NULL, NULL);
    ...
    if (!strcmp(tree->name, "Compiler"))
    {
        ...
        return TAC(tree->lchild, scope, stack);
    }
    else if (!strcmp(tree->name, "Decl")) {
        return TAC(tree->lchild, scope, stack);
    }
    else if (!strcmp(tree->name, "ConstDecl"))
    {
        return MergeTACItem(TAC(tree->lchild->rchild->rchild, scope, stack), TAC(tree->lchild->rchild->rchild->rchild, scope, stack));
    }
    else if (!strcmp(tree->name, "ConstDefs"))
    {
        if (tree->rchild->name == "SPSEMICOLON") {
            return TAC(tree->lchild, scope, stack);
        }
        else
            return MergeTACItem(TAC(tree->lchild, scope, stack), TAC(tree->rchild->rchild, scope, stack));
    }
    else if (!strcmp(tree->name, "ConstDef"))
    {
        未完成
    }
    else if (!strcmp(tree->name, "VarDecl"))
    {
        return MergeTACItem(TAC(tree->lchild->rchild, scope, stack), TAC(tree->lchild->rchild->rchild, scope, stack));
    }
    else if (!strcmp(tree->name, "VarDefs"))
    {
        if (!strcmp(tree->rchild->name,"SPSEMICOLON")) {
            return TAC(tree->lchild, scope, stack);
        }
        else
            return MergeTACItem(TAC(tree->lchild, scope, stack), TAC(tree->rchild->rchild, scope, stack));
    }
    else if (!strcmp(tree->name, "VarDef"))
    {
        未完成
    }
    else if (!strcmp(tree->name, "FuncDef"))
    {
        ...
        if (!strcmp(tree->lchild->rchild->rchild->rchild->name, "FuncFParams")) {
            fun.code_table.push_back(0);
            fun.TAC_table[0].seq = 0;
            fun.TAC_table[0].optype = FUNCTIONDF;
            fun.TAC_table[0].firstOp.Type = FUNCTION;
            fun = MergeTACItem(fun, TAC(tree->lchild->rchild->rchild->rchild, scope, stack));
            return MergeTACItem(fun, TAC(tree->lchild->rchild->rchild->rchild->rchild->rchild, scope, stack));
        }
        else {
            fun.code_table.push_back(0);
            fun.TAC_table[0].seq = 0;
            fun.TAC_table[0].optype = FUNCTIONDF;
            fun.TAC_table[0].firstOp.Type = FUNCTION;
            return MergeTACItem(fun, TAC(tree->lchild->rchild->rchild->rchild->rchild, scope, stack));
        }
         
    }
    else if (!strcmp(tree->name, "FuncFParams"))
    {
        if (!strcmp(tree->rchild->name, "OPRIGHTPRNT") {
            return TAC(tree->lchild, scope, stack);
        }
        else
            return c
    ...
    {
        return TAC(tree->lchild->rchild, scope, stack);
    }
    else if (!strcmp(tree->name, "BlockItems"))
    {
        ...
        else return MergeTACItem(TAC(tree->lchild, scope, stack), TAC(tree->lchild->rchild, scope, stack));
    }
    else if (!strcmp(tree->name, "BlockItem"))
    {
        return TAC(tree->lchild, scope, stack);
    }
    ...
    else if (!strcmp(tree->name, "Variable"))
    {
        ...
        var.code_table.push_back(0);
        var.TAC_table[0].seq = 0;
        var.TAC_table[0].optype = PARAM;
        var.TAC_table[0].firstOp.Type = VARIABLE;
        var.TAC_table[0].firstOp.variable = TraverseScopeStack(*stack, tree->lchild->rchild->string_value);
