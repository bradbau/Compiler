...
TACCode* TAC:: MergeTACItem(int num, ...)
{
    int i = 1;
    va_list codelist;
    TACCode* entrance;
...
TACCode* TAC::TranslateStmt(ASTTree* tree, ScopeItem scopeItem, ScopeStack* stack, Label continuevalue, Label breakvalue) {
    if (tree->name == "Empty_Stmt") {
        ...
        scopeItem = *GetStackTopp(stack);
        TACCode* code2 = TranslateStmt(stmt, scopeItem, stack, continuevalue, breakvalue);
        ...
        scopeItem = *GetStackTopp(stack);
        TACCode* code5 = TranslateStmt(stmt->rchild, scopeItem, stack, continuevalue, breakvalue);
        ...
        new_temp = *addIntoScope(Local, scope, cache, Variable, "temp" , NULL);
...
TACCode* TAC::TranslateExps(ASTTree* tree, ScopeItem scopeItem, ScopeStack* stack)
{
    if (tree==NULL) {
        return NULL;
    }
...
TACCode* TAC::TranslateExp(ASTTree* tree, ScopeItem scopeItem, ScopeStack* stack, ScopeItem place)
{
    if (tree->name=="IntConst")   //ok
    {
        ...
        tmp->code.firstOp.variable = *tree->si;
    ...
    else if (tree->name == "funcall")
    {
        ...
        code3->code.firstOp.function = function;
        code3->line = tree->lchild->line;
        code3->prev = code3;
        ...
        tmp->code.dest.variable = place;
        tmp->code.firstOp.Type = INTEGERCONST;
        tmp->code.firstOp.value = 0;
        tmp->code.secondOp.Type = VARIABLE;
        tmp->code.secondOp.variable = new_temp;
        tmp->line = tree->lchild->line;
        tmp->prev = tmp;
        scopeItem = *GetStackTopp(stack);
    ...
    else if (tree->name == "(Exp)")
    {
        ...
        tmp->code.dest.variable = place;
        tmp->code.firstOp.Type = VARIABLE;
        tmp->code.firstOp.variable = new_temp;
        tmp->line = tree->line;
        tmp->prev = tmp;
        scopeItem = *GetStackTopp(stack);
        return MergeTACItem(2, TranslateExp(tree->lchild, new_temp, stack, new_temp), tmp);
...
TACCode* TAC::TranslateArgs(ASTTree* tree, ScopeItem scopeItem, ScopeStack* stack)
{
    if (tree->line == -1)
        return NULL;
    else
...
TACCode* TAC::TranslateCondition(ASTTree* tree, ScopeItem scopeItem, ScopeStack* stack, unsigned int label_true, unsigned int label_false)
{
...
TACCode* TAC:: BuildTAC(ASTTree* tree, ScopeItem scopeItem, ScopeStack* stack)
{
    if (tree == NULL)
        return NULL;
    ...
    else if (tree->name == "FuncDef")
    {
        ...
        tmp->line = tree->line;
        tmp->prev = tmp;
        formalcode = BuildTAC(tree->lchild, scopeItem, stack);                    //形参四元式
        bodycode = BuildTAC(tree->lchild->rchild, scopeItem, stack);              //函数体四元式
        //作用域栈操作部分
        if(tree->lchild->si)
        ...
        scopeItem = *GetStackTopp(stack);
        return TranslateExp(tree->lchild, scopeItem, stack, scopeItem);//这里不知道第四个参数会不会出问题
    }
        ...
        BlockItems = BuildTAC(tree->lchild, scopeItem, stack);
        ScopeItem top;
        if (flag == 1) {
            PopScopeStack(stack, &top);
        }
    ...
    while (tmp)
    {
        ...
        else if (tmp->code.optype == ADD | tmp->code.optype == SUB | tmp->code.optype == MUL | tmp->code.optype == DIV)
        {
            ...
            if (tmp->code.secondOp.Type == VARIABLE)
                printf("%s\n", tmp->code.secondOp.variable.depictor->name.c_str());
